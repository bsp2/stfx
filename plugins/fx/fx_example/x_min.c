...
static void ST_PLUGIN_API loc_process_replace(st_plugin_voice_t  *_voice,
   ...
   for(unsigned int i = 0u; i < _numFrames; i++)
   {
      ...
      float busL = samplesBus[k]      * voice->mod_lvl_bus_cur;
      float busR = samplesBus[k + 1u] * voice->mod_lvl_bus_cur;

      float outL = l * voice->mod_lvl_in_cur;
      float outR = r * voice->mod_lvl_in_cur;

      outL = (outL < busL) ? outL : busL;
      outR = (outR < busR) ? outR : busR;
