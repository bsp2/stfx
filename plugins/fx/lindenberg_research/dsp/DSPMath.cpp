 ...
 */
float getPhaseIncrement(float frq, const float _sampleRate) {
    return TWOPI * frq / _sampleRate;
}
 ...
 */
void LP6DBFilter::updateFrequency(float fc, int factor, const float _sampleRate) {
    LP6DBFilter::fc = fc;
    RC = 1.f / (LP6DBFilter::fc * TWOPI);
    dt = 1.f / _sampleRate * factor;
    alpha = dt / (RC + dt);
}
