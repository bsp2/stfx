...
void dsp::Type35Filter::invalidate() {
    ...
    peak = cubicShape(peak) * 2.f + noise.nextFloat(10e-7f);
...
void dsp::Type35Filter::processHPF() {
    ...
    float u = Ga * (y1 + s35h);
    float y = peak * fastatan(sat * u * 0.1f) * 10.f;
