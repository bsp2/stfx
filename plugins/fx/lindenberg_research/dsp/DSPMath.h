...
public:
     ...
     */
    LP6DBFilter(float fc, int factor, const float _sampleRate) {
       updateFrequency(fc, factor, _sampleRate);
       y0 = 0.f;
    }
     ...
     * @param fc cutoff frequency
     */
   void updateFrequency(float fc, int factor, const float _sampleRate);
